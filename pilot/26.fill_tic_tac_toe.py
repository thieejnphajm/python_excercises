...
def print_vertical(game, row, size):
    vertical = '|'
    for col in range(size):
        if game[row][col] == 0:
            vertical += '   |'
        else:
            vertical += ' {} |'.format(game[row][col])
    print vertical


def print_go(game, size):
    print_horizon(size)

    for row in range(size):
        print_vertical(game, row, size)
        print_horizon(size)

size = int(raw_input("Size of the GO: "))

game = [[0 for _ in range(size)] for _ in range(size)]
...
while True:
    move = raw_input('Next step player {}: '.format(int(playerNo) + 1))
    if move == 'stop':
        break
    moves = [int(i) for i in move.strip().split(' ')]
    game[moves[0]][moves[1]] = 'X' if playerNo else 'O'
